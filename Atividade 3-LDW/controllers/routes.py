...
# Lista de jogos
serielist = [{'Título': 'CS-GO', 'Ano': 2012, 'Genero': 'FPS Online'}]
...
def init_app(app):
        ...
        if request.method == 'POST':
            if request.form.get('titulo') and request.form.get('ano') and request.form.get('genero'):
                serielist.append({'Título': request.form.get('titulo'), 'Ano': request.form.get(
                    'ano'), 'Genero': request.form.get('genero')})
    ...
    def seriesEstoque(id=None):
        ...
        if request.method == 'POST':
            newserie = Serie(request.form['titulo'], request.form['ano'], request.form['genero'],
                           request.form['nome'], request.form['iddae'], request.form['pais'])
    ...
    def serieEdit(id):
        ...
        if request.method == 'POST':
            ...
            s.categoria = request.form['genero']
            # Alterando o Ator
            g.ator_id = request.form['nome']
